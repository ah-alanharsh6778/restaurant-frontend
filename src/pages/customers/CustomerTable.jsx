...
export const CustomerTable = ({
  ...
  const getLoyaltyBadge = (points = 0) => {
    ...
    return <Chip label="Standard (0 pts)" size="small" variant="outlined" sx={{ fontWeight: 700, fontSize: '0.725rem', borderColor: 'rgba(255, 255, 255, 0.2)', color: '#9CA3AF', borderRadius: '4px' }} />;
  };
  ...
  const columns = [
        ...
        return (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 0 }}>
            ...
            <Box sx={{ minWidth: 0, flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <Typography variant="subtitle2" sx={{ fontWeight: 800, color: '#FFFFFF', lineHeight: 1.25, fontSize: '0.9rem' }} noWrap>
                {nameStr}
              </Typography>
              <Typography variant="caption" sx={{ color: '#9CA3AF', lineHeight: 1.2, fontSize: '0.75rem', mt: 0.25 }} noWrap>
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return tableNum ? (
          <Chip label={`Table #${tableNum}`} color="primary" size="small" sx={{ fontWeight: 800, backgroundColor: '#7C6CFF', borderRadius: '4px' }} />
        ) : (
    ...
    },
    {
      ...
      render: (row) => {
        ...
        if (!res) {
          return <Typography variant="caption" sx={{ color: '#9CA3AF' }}>No Active Booking</Typography>;
        }
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return (
          <Typography variant="body2" sx={{ fontWeight: 800, color: count > 0 ? '#7C6CFF' : '#9CA3AF' }}>
  ...
  // Render Table View across all devices (Mobile & Desktop)
  return (
    <Paper
      elevation={0}
      sx={{
